...
void btn1_press(const struct device *dev, struct gpio_callback *cb,
		    uint32_t pins)
{
	//flashLed1();
}

void btn2_press(const struct device *dev, struct gpio_callback *cb,
		    uint32_t pins)
{
        //flashLed1();
}

void btn3_press(const struct device *dev, struct gpio_callback *cb,
		    uint32_t pins)
{
        //flashLed1();
}

void btn4_press(const struct device *dev, struct gpio_callback *cb,
		    uint32_t pins)
{
	//Enter button
}
...
int main(void)
{
	...
	ret = gpio_pin_interrupt_configure_dt(&btn2,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		return 0;
	}

	ret = gpio_pin_interrupt_configure_dt(&btn3,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		return 0;
	}

	ret = gpio_pin_interrupt_configure_dt(&btn4,
					      GPIO_INT_EDGE_TO_ACTIVE);
	if (ret != 0) {
		return 0;
	}

        gpio_init_callback(&button_cb_data, btn1_press, BIT(btn1.pin));
	gpio_add_callback(btn1.port, &button_cb_data);

        gpio_init_callback(&button_cb_data2, btn2_press, BIT(btn2.pin));
	gpio_add_callback(btn2.port, &button_cb_data2);

        gpio_init_callback(&button_cb_data3, btn3_press, BIT(btn3.pin));
	gpio_add_callback(btn3.port, &button_cb_data3);

        gpio_init_callback(&button_cb_data4, btn4_press, BIT(btn4.pin));
	gpio_add_callback(btn4.port, &button_cb_data4);
